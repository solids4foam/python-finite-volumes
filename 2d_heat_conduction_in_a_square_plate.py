...
Tan =AnalyticalHeatConduction(Tt,Tb,Tl,Tr,Lx,Ly,Lx/Nx,Ly/Ny,Nx,Ny)
...
#     plt.contourf(X, Y, T_plot, cmap='inferno')
    ...
    plt.show()
    plt.figure()
    plt.contourf(X,Y,abs(T_anplot-T_fvplot),cmap='inferno')
    plt.colorbar()
    plt.show()
    plt.contour
...
Tfv,mse,l2_norm,inf_norm = JustFVheatConduction(Lx,Ly,Nx,Ny,k,Tl,Tr,Tt,Tb)
...
for i in range(0,4):
    ...
    _,_, mse[i],l2_norm[i], inf_norm[i],python_norm[i] = FVheatConduction(Lx,Ly,Nx,Ny,k,Tl,Tr,Tt,Tb)
    Nx=Nx*2
    Ny=Ny*2
print(l2_norm)
print(mse)
...
abaqus
dt = 5000,10000,20000,40000
total_time = 1,000,000
"""
...
df_abaqus = pd.read_csv("/Users/eoinmolloy/Downloads/AbaqusData.csv")
...
file = df_merged.to_csv('dfFVM.csv')
